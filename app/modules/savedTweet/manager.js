...
class BLManager {
  ...
  async writeSpeed() {
    ...
    let response = [];
    let result = await savingSpeedDataPointsModel.findData({}, {}, 0, 10, {
      addedOn: -1,
    });
  ...
  async maxTpsCount() {
    ...
    let TpsNo = await maxTpsModel.findData({}, selectionKey, 0, 1, {
      tpscount: -1,
    });
    return Math.max.apply(
      Math,
      TpsNo.map(function (o) {
        return o.tpscount;
      })
    );
  }
  ...
  async archivedTweetFromTestNet(request) {
    try {
      if (!request || !request.id) {
        throw apiFailureMessage.INVALID_REQUEST;
      }
      let result = await blockchainResponseModel.findData({ tweetId: request.id }, {}, 0, 10, { addedOn: -1 });
      return result
    } catch (err) {
      throw new Error(err);
    }
  ...
  async archivedTweetAdvanceSearchDetails(request) {
    try {
      if (!request || !request.id) {
        throw apiFailureMessage.INVALID_REQUEST;
      }
      let result = await tweets.findData({ id: request.id },{}, 0, 10, { addedOn: -1 });
      return result
    } catch (err) {
      throw new Error(err);
    }
